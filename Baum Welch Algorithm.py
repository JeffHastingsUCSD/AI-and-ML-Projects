...
def forward(obs_seq, states, start_prob, trans_prob, emis_prob):
    ...
    for s in range(num_states):
        fwd[0, s] = start_prob[s] * emis_prob[s, obs_seq[0]]
    for t in range(1, num_obs):
        for s in range(num_states):
            fwd[t, s] = np.sum(fwd[t-1, :] * trans_prob[:, s]) * emis_prob[s, obs_seq[t]]
    total_prob = np.sum(fwd[num_obs - 1, :])
    return fwd, total_prob

def backward(obs_seq, states, trans_prob, emis_prob):
    num_states = len(states)
    num_obs = len(obs_seq)
...
def baum_welch(obs_seq, states, start_prob, trans_prob, emis_prob, n_iter=100):
    ...
    for _ in range(n_iter):
        ...
        bwd, bwd_prob = backward(obs_seq, states, trans_prob, emis_prob)
        ...
        for s in range(num_states):
            for o in range(emis_prob.shape[1]):
                emis_prob[s, o] = np.sum(gamma[obs_seq == o, s]) / np.sum(gamma[:, s])
